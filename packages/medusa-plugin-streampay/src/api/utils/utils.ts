import {
  ...
  const PAYMENT_PROVIDER_KEY = "pp_streampay"
  ...
  export function constructWebhook({
    ...
    const streampayProviderService = container.resolve(PAYMENT_PROVIDER_KEY)
    return streampayProviderService.constructWebhookEvent(body, signature)
  }
  ...
  export function buildError(event: string, err: StreamPay.StreamPayRawError): string {
    let message = `StreamPay webhook ${event} handling failed${EOL}${
      err?.detail ?? err?.message
    }`
    if (err?.code === PostgresError.SERIALIZATION_FAILURE) {
      message = `StreamPay webhook ${event} handle failed. This can happen when this webhook is triggered during a cart completion and can be ignored. This event should be retried automatically.${EOL}${
        err?.detail ?? err?.message
      }`
    }
    if (err?.code === "409") {
      message = `StreamPay webhook ${event} handle failed.${EOL}${
        err?.detail ?? err?.message
      }`
    }
